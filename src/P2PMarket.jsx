// src/P2PMarket.jsx - ПОЛНЫЙ ИСПРАВЛЕННЫЙ
...
export default function P2PMarket({ telegramUser, showToast, onBack, navigateTo }) {
    ...
    const TradeCard = ({ trade, onUpdate, isNew }) => {
        ...
        const getStatusBadge = () => {
            switch (trade.status) {
                case 'pending': return { text: '⏳ Ожидает оплаты', color: '#ff9500', bg: 'rgba(255, 149, 0, 0.15)' };
                case 'paid': return { text: '💳 Оплачено', color: '#34c759', bg: 'rgba(52, 199, 89, 0.15)' };
                case 'completed': return { text: '✅ Завершена', color: '#34c759', bg: 'rgba(52, 199, 89, 0.1)' };
                case 'cancelled': return { text: '❌ Отменена', color: '#ff3b30', bg: 'rgba(255, 59, 48, 0.1)' };
                case 'expired': return { text: '⏰ Просрочена', color: '#8e8e93', bg: 'rgba(142, 142, 147, 0.1)' };
                default: return { text: trade.status, color: '#888', bg: 'rgba(136, 136, 136, 0.1)' };
            }
        };
        ...
        return (
            <div className={`trade-card-new ${trade.status} ${showHighlight ? 'trade-card-highlight' : ''}`}>
                <div className="trade-card-header">
                    ...
                    <div className="trade-status-badge" style={{ background: badge.bg, color: badge.color }}>{badge.text}</div>
                </div>
                ...
                <div className="trade-actions-new">
                    {trade.status === 'pending' && isBuyer && (
                        <button className="action-btn pay-btn" onClick={() => confirmPayment(trade.trade_id)}>💳 Оплатил</button>
                    )}
                    {trade.status === 'paid' && !isBuyer && (
                        <button className="action-btn confirm-btn" onClick={() => confirmReceipt(trade.trade_id)}>✅ Получил</button>
                    )}
                    {(trade.status === 'pending' || trade.status === 'paid') && (
                        <button className="action-btn cancel-btn" onClick={() => cancelTrade(trade.trade_id)}>❌ Отменить</button>
                    )}
                    {showChat && (
                        <button className="action-btn chat-btn" onClick={() => setSelectedTrade(trade)}>💬 Чат</button>
                    )}
                </div>
            </div>
        );
        ...
        <div className="modal" onClick={() => setSelectedTrade(null)}>
            <div className="modalContent chat-modal" onClick={e => e.stopPropagation()}>
                ...
                <div className="chat-messages">
                    {messages.length === 0 ? <div className="chat-empty">Нет сообщений. Начните диалог!</div> : messages.map((msg) => (
                ...
                <div className="chat-input-container">
                    <input type="text" className="chat-input" placeholder="Напишите сообщение..." value={newMessage} onChange={(e) => setNewMessage(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && sendMessage()} />
                    <button className="chat-send" onClick={sendMessage} disabled={sendingMessage || !newMessage.trim()}>{sendingMessage ? '⏳' : '➤'}</button>
                </div>
            </div>
        </div>
    );
        ...
        <div className="modal" onClick={() => setSelected(null)}>
            <div className="modalContent" onClick={e => e.stopPropagation()}>
                ...
                <div className="modalBody">
                    ...
                    <input ref={amountInputRef} type="number" className="amountInput" value={amount} onChange={e => setAmount(e.target.value)} placeholder={`Введите сумму в ${currencyType === 'usdt' ? 'USDT' : 'RUB'}`} />
    ...
    const ProfileScreen = () => {
        ...
        return (
            <div className="profile">
                ...
                <div className="stats">
                    <div><b>{completedTrades}</b><span>✅ Выполнено</span></div>
                    <div><b>{successRate}%</b><span>📊 Успешных</span></div>
    ...
    const BuyScreen = () => {
        ...
        return (
            <div className="screen">
                <div className="header"><button onClick={() => setScreen('main')}>←</button><h2>Купить USDT</h2><div></div></div>
                <StaticBanner />
                <div className="filter-bar">
                    <div className="filter-chip">
                        <div className="filter-chip-label">Способ</div>
                        <div className="filter-chip-value" onClick={() => setShowPaymentDropdown(!showPaymentDropdown)}>{selectedPayment.label}<span className="chip-arrow">⌄</span></div>
                    ...
                    <div className="filter-chip">
                        ...
                        <div className="filter-chip-value" onClick={() => setShowTimeDropdown(!showTimeDropdown)}>{selectedTime.label}<span className="chip-arrow">⌄</span></div>
    ...
    const SellScreen = () => {
        ...
        return (
            <div className="screen">
                <div className="header"><button onClick={() => setScreen('main')}>←</button><h2>Продать USDT</h2><div></div></div>
                <StaticBanner />
                <div className="filter-bar">
                    <div className="filter-chip">
                        <div className="filter-chip-label">Способ</div>
    ...
    const startEdit = (ad) => {
        ...
        setEditForm({
            ...
            terms: ad.terms || '',
            payment_time: ad.payment_time || '30'
        });
    };
    ...
    const updateAd = async () => {
        ...
        try {
            const res = await fetch(`${API}/api/p2p/order/${editingAd.id}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    userId: userId,
                    rate: parseFloat(editForm.rate),
                    min_amount: parseFloat(editForm.min_amount) || editingAd.min_amount,
                    max_amount: parseFloat(editForm.max_amount) || editingAd.max_amount,
                    payment_methods: editForm.payment_methods,
                    terms: editForm.terms,
                    payment_time: parseInt(editForm.payment_time),
    ...
    return (
                        ...
                        <div key={ad.id} className="ad-card">
                            {editingAd?.id === ad.id ? (
                                <div className="edit-form">
                                    ...
                                    <input type="number" placeholder="Курс (RUB)" value={editForm.rate} onChange={e => setEditForm({...editForm, rate: e.target.value})} />
                                    <div className="row">
                                        <input type="number" placeholder="Мин. сумма" value={editForm.min_amount} onChange={e => setEditForm({...editForm, min_amount: e.target.value})} />
                                        <input type="number" placeholder="Макс. сумма" value={editForm.max_amount} onChange={e => setEditForm({...editForm, max_amount: e.target.value})} />
                                    </div>
                                    <div className="paymentBtns">
                                        {paymentMethodsList.map(m => (
                                            <button key={m.value} className={editForm.payment_methods.includes(m.value) ? 'selected' : ''} onClick={() => togglePaymentMethod(m.value)}>
                                                {m.icon}
                                            </button>
                                        ))}
                                    </div>
                                    <textarea placeholder="Условия сделки" value={editForm.terms} onChange={e => setEditForm({...editForm, terms: e.target.value})} rows="2" />
                                    <select value={editForm.payment_time} onChange={e => setEditForm({...editForm, payment_time: e.target.value})}>
                                        {timeOptions.map(opt => <option key={opt.value} value={opt.value}>⏰ {opt.label}</option>)}
                                    </select>
                                ...
                                <>
                                    ...
                                    <div className="ad-actions">
                                        <button className="ad-edit" onClick={() => startEdit(ad)}>✏️ Редактировать</button>
                                        <button className="ad-share" onClick={() => shareAd(ad)}>📤 Поделиться</button>
                                        <button className="ad-delete" onClick={() => { 
                                            if(window.confirm('Удалить объявление?')) 
                                                fetch(`${API}/api/p2p/order/${ad.id}`, {method:'DELETE', headers:{'Content-Type':'application/json'}, body:JSON.stringify({userId})}).then(()=>{
        ...
        <div className="ad-header">
            <span className={`ad-type ${ad.type}`}>{ad.type === 'sell' ? 'Продажа' : 'Покупка'}</span>
            <button className="edit-cancel" onClick={(e) => { e.stopPropagation(); cancelEdit(); }}>✕</button>
        </div>
        <input 
            type="number" 
        ...
        <div className="paymentBtns" onClick={(e) => e.stopPropagation()} onMouseDown={(e) => e.stopPropagation()}>
            {paymentMethodsList.map(m => (
                <button 
                    key={m.value} 
                    className={editForm.payment_methods.includes(m.value) ? 'selected' : ''}
                    onClick={(e) => { e.stopPropagation(); togglePaymentMethod(m.value); }}
                >
                    {m.icon}
                </button>
            ))}
        ...
        <div className="edit-actions" onClick={(e) => e.stopPropagation()} onMouseDown={(e) => e.stopPropagation()}>
            <button className="edit-save" onClick={(e) => { e.stopPropagation(); updateAd(); }}>💾 Сохранить</button>
            <button className="edit-cancel-btn" onClick={(e) => { e.stopPropagation(); cancelEdit(); }}>❌ Отмена</button>
        </div>
    </div>
) : (
    // обычный вид
